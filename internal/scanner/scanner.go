...
func (s *BlockScanner) processBlock(ctx context.Context, blockNum int64) error {
	...
	s.logger.Info("Processing block", zap.Int64("block", blockNum), zap.Int("tx_count", len(block.Transactions)))
	...
	for _, tx := range block.Transactions {
		...
		if err != nil {
			s.logger.Warn("Failed to parse transfer data", zap.Int64("block", blockNum), zap.String("tx_hash", tx.Hash), zap.Error(err))
			continue
		}
		...
		if !tron.IsValidTronAddress(from) || !tron.IsValidTronAddress(to) {
			s.logger.Warn("Invalid address format", zap.Int64("block", blockNum), zap.String("tx_hash", tx.Hash), zap.String("from", from), zap.String("to", to))
			continue
		}
		...
		// 获取log_index
		logIndexes := logIndexMap[tx.Hash]
		logIndex := int32(0)
		if len(logIndexes) > 0 {
			logIndex = logIndexes[0] // 如有多个log_index可遍历写多条
		}
		...
		if err != nil {
			s.logger.Error("Failed to save records", zap.Int64("block", blockNum), zap.String("tx_hash", tx.Hash), zap.Error(err))
			continue
		}

		s.logger.Info("Transaction processed", zap.Int64("block", blockNum), zap.String("hash", tx.Hash), zap.String("from", from), zap.String("to", to), zap.String("amount", adjustedAmount.String()))
	}
